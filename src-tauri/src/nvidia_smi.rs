...
fn inspect_command() -> BackendResult<ResizableBarInspection> {
    ...
    let evidence = build_evidence(String::new(), executable, tool, &xml, &devices)?;
    Ok(build_inspection(&evidence, &devices))
}
...
fn build_inspection(evidence: &NvidiaSmiEvidence, devices: &[GpuDevice]) -> ResizableBarInspection {
...
<nvidia_smi_log>
  ...
  <gpu id="00000000:01:00.0">
    ...
    fn inspection_classifies_expanded_and_exact_legacy_apertures() {
        ...
        let expanded_gpu = test_gpu(1, EXPANDED);
        ...
        let legacy_gpu = test_gpu(2, LEGACY);
